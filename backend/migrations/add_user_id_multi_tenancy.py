...
def get_business_tables(conn):
    ...
    system_tables = {
        ...
        'tenant_usage_stats', 'tenants', 'sqlite_sequence'
    }
...
def add_user_id_to_table(conn, table_name):
    ...
    try:
        ...
        try:
            cursor.execute(f"""
                CREATE TRIGGER fk_{table_name}_user_id 
                AFTER INSERT ON {table_name}
                BEGIN
                    SELECT CASE
                        WHEN NEW.user_id IS NOT NULL AND NOT EXISTS(SELECT 1 FROM users WHERE id = NEW.user_id)
                        THEN RAISE(ABORT, 'Foreign key constraint failed: user_id')
                    END;
                END
            """)
        except Exception as e:
            print(f"   ⚠️  Could not add FK constraint for {table_name}: {e}")
...
def create_migration_log(conn, tables_processed, success_count, error_count):
    ...
    try:
        ...
        cursor.execute("""
            ...
            (migration_name, tables_processed, success_count, error_count, notes)
            VALUES (?, ?, ?, ?, ?)
        """, (
            ...
            success_count,
            error_count,
            'Added user_id column to all business tables for multi-tenancy'
        ))
        
        conn.commit()
        print(f"📝 Migration log created")
...
def main():
    ...
    try:
        ...
        create_migration_log(conn, tables_processed, success_count, error_count)
