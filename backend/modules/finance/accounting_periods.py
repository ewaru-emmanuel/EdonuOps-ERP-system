...
class FiscalYear(db.Model):
    ...
    year = db.Column(db.Integer, nullable=False, unique=True)  # e.g., 2024
    ...
    @classmethod
    def get_current_fiscal_year(cls, user_id: int = None):
    ...
    @classmethod
    def create_default_fiscal_year(cls, year: int, user_id: int = None):
        """Create a default fiscal year (Jan 1 - Dec 31)"""
        ...
        if user_id:
            ...
            cls.query.filter_by(is_current=True).update({'is_current': False})
        ...
        AccountingPeriod.create_monthly_periods(fiscal_year.id, user_id)
...
class AccountingPeriod(db.Model):
    ...
    @classmethod
    def get_current_period(cls, user_id: int = None):
    ...
    @classmethod
    def get_period_for_date(cls, transaction_date: date, user_id: int = None):
    ...
    @classmethod
    def create_monthly_periods(cls, fiscal_year_id: int, user_id: int = None):
...
class PeriodManager:
    ...
    def initialize_default_periods(self, user_id: int = None):
        """Initialize default fiscal year and periods for a user"""
        current_year = datetime.now().year
        
        # Check if fiscal year already exists
        existing_fy = FiscalYear.query.filter_by(year=current_year, user_id=user_id).first()
        if existing_fy:
            return existing_fy
        
        # Create default fiscal year with monthly periods
        fiscal_year = FiscalYear.create_default_fiscal_year(current_year, user_id)
        db.session.commit()
        
        return fiscal_year
    
    def get_current_period(self, user_id: int = None) -> Optional[AccountingPeriod]:
        """Get the current accounting period"""
        return AccountingPeriod.get_current_period(user_id)
    
    def get_period_for_date(self, transaction_date: date, user_id: int = None) -> Optional[AccountingPeriod]:
        """Get the accounting period for a specific date"""
        return AccountingPeriod.get_period_for_date(transaction_date, user_id)
    
    def validate_transaction_date(self, transaction_date: date, user_id: int = None) -> Tuple[bool, str, Optional[AccountingPeriod]]:
        """
        Validate if a transaction can be created for the given date
        Returns (is_valid, message, period)
        """
        period = self.get_period_for_date(transaction_date, user_id)
        if not period:
    ...
    def close_period(self, period_id: int, user_id: int, reason: str = "Period closed"):
        ...
        period.lock_period(user_id, reason)
    ...
    def get_period_summary(self, user_id: int = None) -> Dict:
        """Get a summary of all periods for a user"""
        ...
        current = self.get_current_period(user_id)
        if current:
